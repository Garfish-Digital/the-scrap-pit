...
function collect(scope: HTMLElement): Round[] {
  return Array.from(scope.querySelectorAll<HTMLElement>('[data-round]')).map((el) => ({
    el,
    entered: false,
    eyebrows: Array.from(el.querySelectorAll<HTMLElement>('.eyebrow')),
    headings: Array.from(el.querySelectorAll<HTMLElement>('h1, h2')).filter((h) => !h.closest('[data-rise]') && !h.classList.contains('eyebrow')),
    slams: Array.from(el.querySelectorAll<HTMLElement>('[data-slam]')),
    ropes: Array.from(el.querySelectorAll<HTMLElement>('.section-head, [data-rope]')),
    rises: Array.from(el.querySelectorAll<HTMLElement>('[data-rise]')).map((g) => Array.from(g.children) as HTMLElement[]),
    odometers: Array.from(el.querySelectorAll<HTMLElement>('[data-odometer]')),
    heroMedia: Array.from(el.querySelectorAll<HTMLElement>('[data-hero-media]')).map((m) => ({
...
function hide(r: Round) {
  gsap.set([...r.eyebrows, ...r.slams], { '--slam-text': 0, '--slam-x': 0, '--slam-o': 'left' })
  gsap.set(r.headings, { autoAlpha: 0, fontVariationSettings: "'wdth' 125" })
  ...
  r.odometers.forEach((o) => gsap.set(o.querySelectorAll('.odo__strip'), { '--odo-y': o.dataset.odometer === 'down' ? '-90%' : '0%' }))
...
function enter(r: Round) {
  ...
  r.heroMedia.forEach((m) => {
    const sat = { v: 0 }
    tl.to([m.tl, m.br], { xPercent: 0, yPercent: 0, duration: 0.3, ease: 'power4.in' }, 0)
      .to(m.img, { y: 3, duration: 0.05, ease: 'power4.in' }, 0.3)
      .to(m.img, { y: 0, duration: 0.25, ease: 'power3.out' }, 0.35)
      .to(sat, { v: 1, duration: 0.9, ease: 'power2.out', onUpdate: () => { m.img.style.filter = `grayscale(${1 - sat.v})` } }, 0.3)
      .to(m.img, { scale: 1.0, duration: 18, ease: 'none' }, 0.3)
  })
  // Images open through the void: a square aperture expands to full bleed.
  tl.to(r.voids, { clipPath: 'inset(0% 0%)', duration: 1.1, ease: 'expo.out', stagger: 0.1 }, 0.15)
  r.eyebrows.forEach((e, i) => slam(tl, e, i * 0.1))
  tl.to(r.ropes, { '--rope': 1, duration: 1.1, ease: 'expo.out' }, 0.05)
  // SETTLE — headings tighten along the width axis
  tl.to(r.headings, { autoAlpha: 1, fontVariationSettings: "'wdth' 62", duration: 1.1, ease: 'expo.out', stagger: 0.08 }, 0.1)
    .set(r.headings, { clearProps: 'fontVariationSettings' }, 1.3)
  r.slams.forEach((e, i) => slam(tl, e, 0.45 + i * 0.12))
  r.rises.forEach((kids, g) => {
    tl.to(kids, { opacity: 1, y: 0, duration: 0.9, ease: 'expo.out', stagger: 0.08 }, 0.5 + g * 0.1)
  })
  ...
  r.odometers.forEach((o, n) => {
    ...
    strips.forEach((strip, i) => {
      tl.to(strip, { '--odo-y': strip.style.getPropertyValue('--odo-final') || '0%', duration: 1.1, ease: 'expo.out' }, at + i * 0.06)
    })
    tl.fromTo(o, { x: 0 }, { x: 1, duration: 0.04, yoyo: true, repeat: 1, ease: 'power4.in' }, at + strips.length * 0.06 + 0.55)
  })
  return tl
}

export function useRounds(scope: RefObject<HTMLElement | null>, routeKey: string) {
