...
**Status (September 12, 2026): studies delivered, decisions pending.** Three routes, published with the site so the design process can be viewed on any device (own lazy chunk; remove before client handoff): `/studies/type`, `/studies/color`, `/studies/logo`. Source in `src/studies/`; logo SVGs in `design/scrap-pit-design/logo/`.
...
Decisions (September 13, 2026):
1. **Type system: B (Archivo) — approved.** Roles: display Archivo 800 / width 62; body 400 / width 100; labels 600 / width 100 tracked caps; numerals 700 / width 112 tabular; quotes 500 / width 88. Self-host and subset (latin) when applied.
2. **Color proposal — approved** as documented on `/studies/color`.
3. **Logo — direction settled on D “Pit Frame” (owner's concept, v2), pending final confirmation.** The owner iterated from a diagonally split square (`public/scrap-pit-logo-4.svg`) to a square frame with a central void torn into two brackets (`public/scrap-pit-logo-5.svg`). It is in the study as **D · Pit Frame**, cleaned: two closed polygons on the 64 grid, transparent void and gap (no ghost-white fills or hairline strokes), Red 600 over Ink, with a dark-surface variant (Ink bracket → Ghost White). Files: `design/scrap-pit-design/logo/d-split-symbol.svg`, `d-split-symbol-dark.svg`, `d-split-lockup.svg` (wordmark in Archivo 800/62). **C · Cage Bar is retired** — the owner judged a typographic signature at odds with the vision. A and B remain on record as explorations. Open detail: test the 16px favicon at 1× and 2× and widen the gap to 6 units if the tear disappears.

**Phase 2 closed (September 13, 2026).** Applied site-wide: Archivo self-hosted (latin + latin-ext, upright + italic; Bebas Neue and Space Mono removed), the approved palette as `tokens.css` with semantic aliases that `.dark` re-points, the four type roles (`.display`, body, `.label`, `.num`, `.quote`) across every page, and the Pit Frame mark in the header, mobile menu, and footer via `src/components/Logo.tsx`. New brand assets: `public/favicon.svg` (dark-chrome aware), `favicon-32.png`, `apple-touch-icon.png`, and a regenerated `og-image.jpg` (source in `design/scrap-pit-design/logo/og-image-source.html`). The studies keep the v1 faces and pigments as local values so they remain an honest record. Build, lint, functional checks, and desktop/mobile screenshots all pass. One composition change worth noting: in Archivo the home hero title now stacks THE / SCRAP / PIT on three lines at desktop, which keeps it clear of the figures; judge on a real display.
...
**Status (September 13, 2026): direction proposed, signature moves prototyped.** The motion score is `design/scrap-pit-design/MOTION.md`: three characters — *The Tear* (geometric, from the mark's diagonal), *Weigh-in* (cinematic; long settles punctuated by hits), *Scoreboard* (mechanical; counters, rails, tickers) — with a recommended hybrid assigned by altitude (page / scene / detail / control), shared micro-interactions, and rules (hit ≤ 300 ms, settle ≥ 600 ms; eases; scrub-or-one-shot; reduced-motion; animate only transform/opacity/clip-path/font-variation-settings). Six working GSAP 3.15 prototypes at `/studies/motion` (`src/studies/motion/`): the Cut (route transition that resolves into the Pit Frame with the destination glimpsed through the void), the Weigh-in hero (bracket snap, three title-line hits that add colour, width-axis subtitle), the Round (eyebrow slam, odometer numeral, rope, baseline-mask lines), odometers + the spoken width-axis quote, Cut & Slip image hover, and the Impact button. GSAP lives in the studies chunk only; the main bundle is unchanged.

**Decisions (September 14, 2026):** hybrid confirmed via per-sequence choices recorded in `design/scrap-pit-design/MOTION.md` ("Owner decisions"): the Cut for routes; hero title lines on an ease-in slam (no skew) with wider spacing; the width-axis reveal for headings (all h2s); slam-flash on eyebrows *and* paragraphs; rope draw kept; baseline mask dropped; odometers on every number with direction by meaning (high rolls up, low rolls down); spoken quote for the three fighter quotes only; Cut & Slip on every non-hero image with scroll-driven playback on touch devices; Impact button static = solid, hover = ghost white slams in and holds, press unchanged. Prototypes at `/studies/motion` reflect the revisions.

**Page altitude built (September 14, 2026).** `src/motion/` holds the live motion layer: `gsap.ts` (entry, reduced-motion helper, `pit:boot` / `pit:reveal` events), `pit-geometry.ts` (shared clip-path math), and `PitOverlay.tsx` + `pit-overlay.css` (one fixed overlay used by both sequences, mounted in `Layout`). The **preloader** runs once per session: an inline script in `index.html` paints the closed overlay before React, the void punches open into the mark, holds, then swallows the viewport. The **Cut** intercepts every internal link at the document capture phase, converges, swaps the route behind the overlay, opens the void with the destination glimpsed, and slides the brackets off. Verified headless: boot frames, cut frames, session gating, same-route anchors bypass, reduced-motion bypass, zero console errors. GSAP core adds ~28 KB gzip to the main bundle. Back/forward navigation does not play the Cut (router has already moved when `popstate` fires).

**Scene + control altitude, first pass (September 15, 2026).** The home hero runs the **Weigh-in** (`src/motion/heroWeighIn.ts`): parts are hidden in a `useLayoutEffect` before first paint and the timeline starts on `pit:boot` / `pit:reveal` through `useReveal` (500 ms fallback covers back/forward arrivals). Title lines are per-word masks; brackets are the mark's two pieces at scene scale in the media pane. The live **Button** now carries the **Impact** language (`src/motion/useImpact.ts`): static solid, ghost-white slam on hover, the approved press. Reduced motion: hero static, buttons use a plain inverted hover. Verified headless on the boot path, the Cut path, the fallback path, hover, and reduced motion; zero console errors.

**Section entrances built (September 15, 2026).** `src/motion/useRounds.ts` + `rounds.css`, mounted once in `Layout` and re-collected per route: every `[data-round]` element enters once from ScrollTrigger (`top 78%`) after the overlay reveals the route — eyebrow slam-flash, rope draw, width-axis `h1`/`h2` reveal, `[data-slam]` paragraph flashes, `[data-rise]` group settles. Hidden state is applied via CSS custom properties in a layout effect, so with no JS or reduced motion every fallback is the resting, visible state. All five routes are marked up (the split heroes, each Gauntlet discipline, pricing and extras, the guarantee, the fighter voices, bridge, leaders, invitation, About's story/numbers/values/CTA, Contact's opener, form and info blocks). Verified headless: Cut arrival, scroll entry, hash arrival to `#guarantee`, nothing left hidden after a scroll-through, reduced motion, zero console errors.

**Detail layer built (September 15, 2026).** `components/Odometer.tsx` (digits as 0–9 strips, `direction` up/down, rolled by the enclosing Round; CSS fallback rests on the final digit) is on About's numbers and value indices, Training's plan prices and extras (down), and the guarantee's 30 (up). `components/SlipImage.tsx` + `motion/useSlips.ts` replace every non-hero image: hover/focus on pointer devices, ScrollTrigger middle-band playback on touch devices, reversing on exit. Verified headless: prices land on the right digits, About numbers roll up, hover slip on the gateway, touch emulation opens only the card in the middle band, zero console errors.

**Interior heroes and void reveals built (September 15, 2026).** `components/PitBrackets.tsx` is the shared bracket pair (Home now uses it too). `[data-hero-media]` on the Training/Fighters split-hero panes and the About backdrop runs a Weigh-in variant inside the section's Round (brackets snap, one jolt, colour settles, drift). `[data-void]` on each Gauntlet discipline's media opens the image through a square aperture. **Pinned Gauntlet: not built** — the score's rule 4 and reading cost outweigh it now that each discipline enters as its own round; recorded in MOTION.md. Verified headless on the Cut path and scroll path; zero console errors.

All five phases are complete. What remains is the owner's own pass on a real phone and display, replacement photography for the badged placeholders, and the `VITE_STUDIES=off` switch on the client-facing deploy. Tuning numbers (bracket scale, hit spacing, trigger point, tear width) are all single values named in `design/scrap-pit-design/MOTION.md`.
...
**Status (September 15, 2026): QA pass complete.** Automated sweep (headless Chrome) across all routes at 320/390/768/1024/1440/1920, plus reduced-motion, keyboard, back/forward, and a 4× CPU-throttled scroll of Training at phone width.

Found and fixed:
- Horizontal overflow on Home at every width (bracket bleed; `.home-hero` now clips) and on About's numbers band at 320/390/1024 (odometer digits can't wrap; sized with container units).
- Header at 768px: six labels overflowed the row; the full menu now takes over below 1024px.
- Keyboard: un-entered sections were `visibility: hidden`, so Tab skipped every button into the footer. Rise groups now hide with opacity only and any round enters on `focusin`. Added a skip link (`#main` is focusable).
- Back/forward forced scroll-to-top; `ScrollManager` now leaves POP navigations to the browser so position is restored.
- Placeholder heroes: 3.1 MB of PNG/JPG replaced by WebP derivatives (300 KB total); originals and the unused candidates (including the recognizable-athlete photo) moved to `design/scrap-pit-design/photo-sources/` so nothing unreferenced deploys.
- Phone `tel:` reconciled with the vanity number; footer year is dynamic; the guarantee copy duplicate from v1 no longer exists in v2.
- Studies kill-switch: `VITE_STUDIES=off` drops the routes and header links (`src/config.ts`).

Verified clean: zero console errors on every route/width; one `h1` per route; every image has `alt`; per-route title/description/`og:url`; reduced motion leaves nothing hidden on any route; Escape closes the mobile menu; 0 long tasks during a throttled scroll of Training (headless proxy — confirm on a real phone).

Owner's post-build adjustments (same day) are applied and listed in `design/scrap-pit-design/MOTION.md` → "Owner adjustments": solid buttons everywhere, Home CTA changes, steel slams (not on hero ledes), line-stable heading reveal, spoken fighter quotes, roomier sections, no "demo" language in the UI.

Remaining, by design: the three flagged placeholder photos stay badged until replaced; `noindex` stays on; the studies stay published until the client deploy sets the switch. Demo-only behaviour is listed in `README.md`.
