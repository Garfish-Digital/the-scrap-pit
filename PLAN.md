...
**Status (September 12, 2026): studies delivered, decisions pending.** Three routes, published with the site so the design process can be viewed on any device (own lazy chunk; remove before client handoff): `/studies/type`, `/studies/color`, `/studies/logo`. Source in `src/studies/`; logo SVGs in `design/logo/`.
...
- Placeholder heroes: 3.1 MB of PNG/JPG replaced by WebP derivatives (300 KB total); originals kept.
